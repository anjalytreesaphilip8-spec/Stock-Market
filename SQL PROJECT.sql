...
SELECT 
    ROUND(SUM(initial_value * quantity), 2) AS Total_Sell_Value,
    ROUND(SUM(buy_price * quantity), 2) AS Total_Buy_Value,
    ROUND(SUM((initial_value * quantity) - (buy_price * quantity)), 2) AS Trader_Performance
FROM Stocks;

SELECT 
    company_name,
    ROUND(SUM(initial_value * quantity), 2) AS Sell_value,
    ROUND(SUM(buy_price * quantity), 2) AS Buy_value,
    ROUND(SUM((initial_value * quantity) - (buy_price * quantity)), 2) AS Trader_Performance
FROM Stocks
